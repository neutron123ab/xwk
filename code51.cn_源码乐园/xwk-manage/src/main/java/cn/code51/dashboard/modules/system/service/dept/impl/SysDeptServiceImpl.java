package cn.code51.dashboard.modules.system.service.dept.impl;
...
import cn.code51.dashboard.modules.system.dal.mysql.dept.SysDeptMapper;
import cn.code51.dashboard.modules.system.enums.SysErrorCodeConstants;
import cn.code51.dashboard.modules.system.enums.dept.DeptIdEnum;
import cn.code51.dashboard.modules.system.mq.producer.dept.SysDeptProducer;
...
public class SysDeptServiceImpl implements SysDeptService {
    ...
    /**
     * 父学院缓存
     * key：学院编号 {@link SysDeptDO#getParentId()}
     * value: 直接子学院列表
     *
     * 这里声明 volatile 修饰的原因是，每次刷新时，直接修改指向
     */
    ...
    /**
     * 如果学院发生变化，从数据库中获取最新的全量学院。
     * 如果未发生变化，则返回空
     *
     * @param maxUpdateTime 当前学院的最大更新时间
     * @return 学院列表
     */
    private List<SysDeptDO> loadDeptIfUpdate(Date maxUpdateTime) {
        // 第一步，判断是否要更新。
        if (maxUpdateTime == null) { // 如果更新时间为空，说明 DB 一定有新数据
            log.info("[loadMenuIfUpdate][首次加载全量学院]");
        } else { // 判断数据库中是否有更新的学院
            if (!deptMapper.selectExistsByUpdateTimeAfter(maxUpdateTime)) {
                return null;
            }
            log.info("[loadMenuIfUpdate][增量加载全量学院]");
        }
        // 第二步，如果有更新，则从数据库加载所有学院
    ...
    /**
     * 递归获取所有的子学院，添加到 result 结果
     *
     * @param result 结果
     * @param parentId 父编号
     * @param recursiveCount 递归次数
     * @param parentDeptMap 父学院 Map，使用缓存，避免变化
     */
