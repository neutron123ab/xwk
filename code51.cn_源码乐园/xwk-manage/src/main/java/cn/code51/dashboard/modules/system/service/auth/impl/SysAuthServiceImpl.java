package cn.code51.dashboard.modules.system.service.auth.impl;
...
import cn.code51.dashboard.framework.tracer.core.util.TracerUtils;
...
import cn.code51.dashboard.modules.system.enums.logger.SysLoginLogTypeEnum;
import cn.code51.dashboard.modules.system.service.auth.SysAuthService;
...
import cn.code51.dashboard.modules.system.controller.auth.vo.auth.SysAuthLoginReqVO;
import cn.code51.dashboard.modules.system.convert.auth.SysAuthConvert;
import cn.code51.dashboard.modules.system.enums.logger.SysLoginResultEnum;
import cn.code51.dashboard.modules.system.service.common.SysCaptchaService;
...
public class SysAuthServiceImpl implements SysAuthService {
    ...
    private void verifyCaptcha(String username, String captchaUUID, String captchaCode) {
        ...
        if (code == null) {
            ...
            throw ServiceExceptionUtil.exception(SysErrorCodeConstants.AUTH_LOGIN_CAPTCHA_NOT_FOUND);
        }
        // 验证码不正确
        if (!code.equals(captchaCode)) {
            // 创建登陆失败日志（验证码不正确)
            this.createLoginLog(username, SysLoginResultEnum.CAPTCHA_CODE_ERROR);
            throw ServiceExceptionUtil.exception(SysErrorCodeConstants.AUTH_LOGIN_CAPTCHA_CODE_ERROR);
        }
        // 正确，所以要删除下验证码
    ...
    private void refreshLoginUserCache(String token, LoginUser loginUser) {
        ...
        if (user == null || CommonStatusEnum.DISABLE.getStatus().equals(user.getStatus())) {
            throw ServiceExceptionUtil.exception(SysErrorCodeConstants.TOKEN_EXPIRED); // 校验 token 时，用户被禁用的情况下，也认为 token 过期，方便前端跳转到登陆界面
        }
