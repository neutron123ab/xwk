package cn.code51.dashboard.modules.infra.controller.file;
...
import cn.code51.dashboard.modules.infra.convert.file.InfFileConvert;
...
public class InfFileController {
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "path", value = "文件路径", required = false, example = "teachsystemyuanma.png", dataTypeClass = String.class)
    })
    public CommonResult uploadFile(@RequestParam("file") MultipartFile file,
                                   @RequestParam("path") String path) throws IOException {
        CommonResult result=fileService.createFile(path, IoUtil.readBytes(file.getInputStream()));
        return result;
    }
